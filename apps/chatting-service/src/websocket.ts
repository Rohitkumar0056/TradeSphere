...
export async function createWebSocketServer(server: HttpServer) {
    ...
    wss.on("connection", (ws: WebSocket) => {
        ...
        let registerUserId: string | null = null;
        ...
        ws.on("message", async (rawMessage) => {
            try {
                ...
                if(!registerUserId && !messageStr.startsWith("{")) {
                    registerUserId = messageStr;
                    connectedUsers.set(registerUserId, ws);
                    console.log(`registered websocket for userId: ${registerUserId}`);

                    const isSeller = registerUserId.startsWith("seller_");
                    const redisKey = isSeller ? `online:seller:${registerUserId.replace("seller_", "")}` : `online:user:${registerUserId}`;
                ...
                //if it's seen update
                if(data.type === "MARK_AS_SEEN" && registerUserId) {
                    const seenKey = `${registerUserId}_${data.conversationId}`;
        ...
        ws.on("close", async () => {
            if(registerUserId) {
                connectedUsers.delete(registerUserId);
                console.log(`Disconnected user ${registerUserId}`);
